...
def make_transaction_p2p_received(trans_id):
    ...
    p2p_info['time_received_p2p'] = make_noisy_date(datetime.now()).strftime("%Y-%m-%d %H:%M:%S")
    return p2p_info
...
def make_transaction_p2b_received(trans_id):
    ...
    p2b_info['time_received_p2b'] = make_noisy_date(datetime.now()).strftime("%Y-%m-%d %H:%M:%S")
    p2b_info['bank'] = fake.company()[:29]
    p2b_info['payee_real_fullname'] = fake.name()[:30]
...
with DAG(
    ...
    transactions = [handle_transaction(make_transaction(client_id)) for client_id, trans_type in zip(verified_clients_ids, generated_trans_types)]
    transactions = sorted(transactions, key=lambda t: t['trans_time_created'])

    transactions_sql = "".join([
        f"INSERT INTO Transaction_extended (client_id, payee_phone, payee_fullname, is_received, trans_time_created, trans_amount, trans_amount_rubles, commission, currency_from, currency_to, exchange_rate, country_from, country_to)"
        f" VALUES ({t['client_id']}, '{t['payee_phone']}', '{t['payee_fullname']}', {t['is_received']}, '{t['trans_time_created']}', {t['trans_amount']}, {t['trans_amount_rubles']}, {t['commission']}, '{t['currency_from']}', '{t['currency_to']}', {t['exchange_rate']}, '{t['country_from']}', '{t['country_to']}');"
    ...
    if len(trans_unreceived_ids) > 0:
        ...
        p2p_received_sqls = "".join([
            f"INSERT INTO p2p_trans (transaction_id, payee_card_hash, time_received_p2p)"
            f" VALUES ({p2p_t['transaction_id']}, '{p2p_t['payee_card_hash']}', '{p2p_t['time_received_p2p']}');"
    ...
    if len(trans_unreceived_ids) > 0:
        ...
        p2b_received_sqls = "".join([
            f"INSERT INTO p2b_trans (transaction_id, time_received_p2b, bank, payee_real_fullname, payee_real_phone)"
            f" VALUES ({p2b_t['transaction_id']}, '{p2b_t['time_received_p2b']}', '{p2b_t['bank']}', '{p2b_t['payee_real_fullname']}', '{p2b_t['payee_real_phone']}');"
